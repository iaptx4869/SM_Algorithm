...
class SM2Utils {
    private static byte[] encrypt(byte[] publicKey, byte[] data) throws IOException {
    ...
    private static byte[] sign(byte[] userId, byte[] privateKey, byte[] sourceData) {
        if (privateKey == null || privateKey.length == 0) {
            return null;
        }
    ...
    private static boolean verifySign(byte[] userId, byte[] publicKey, byte[] sourceData, byte[] signData) throws IOException {
        if (publicKey == null || publicKey.length == 0) {
            return false;
        }
